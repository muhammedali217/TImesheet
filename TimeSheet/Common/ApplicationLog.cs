    ...
    {  
        ...
        {            
            if(exMe.Message.ToString() !="Attempted to divide by zero.")
                SendEMail(FormatOrdinaryException(exMe.Message)); 
            else
                SendEMail(exMe.Message);
        }

        public static void LogError(string strExeptionMessage)
        {
            LogError(strExeptionMessage, null);
        }

        public static void LogError(string strExeptionMessage, string strSubject)
        {
            if (strSubject != null)
                SendEMail(strExeptionMessage, strSubject);
            else
                SendEMail(strExeptionMessage, "Error in Controller TechV ERP");
        }

        public static void LogError(WebException exMe)
        {
            if (exMe.Message.ToString() != "Attempted to divide by zero.")
                SendEMail(FormatOrdinaryException(exMe.Message));
            else
        ...
        private static string FormatOrdinaryException(string strMessage)
        {           
            StringBuilder sb = new StringBuilder();             
            sb.Append("Time of Error: " + DateTime.Now.ToString("g") + Environment.NewLine);
            if (strMessage != null)
            {
                sb.Append("Message: " + strMessage + Environment.NewLine);                
        ...
        {
            SendEMail(strText, "Error in Controller, TimeSheet Project");
        }

        private static void SendEMail(string strText, string strSubject)
        {
            ...
            oSmtpClient.Credentials = new NetworkCredential("puretextingteam@gmail.com", "password#123");
