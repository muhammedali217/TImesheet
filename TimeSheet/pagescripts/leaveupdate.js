$(document).ready(function () {
    ...
    $('#btnUpdate').click(function () {
        ...
        if ($("#ddlEmployee").val() == null) {
            ...
            varParams.strArgmt = LeaveType.toString();;
    ...
    $('#btnUpdateSL').click(function () {

        if ($("#ddlEmployee").val() == null) {
            $.alert.open({ type: 'warning', content: 'Please choose atleast one employee' });
        }
        else if ($("#txtPL").val() == "") {
            $.alert.open({ type: 'warning', content: 'Please enter the SL' });
        }
        else {
            var empId = $("#ddlEmployee").val();
            var LeaveType = 'S'
            ...
            var varParams = {};
            varParams.strKey = "Type";
            varParams.strArgmt = LeaveType.toLocaleString();
            varProcParams[2] = varParams;
            varParams = {};
            ...
            success: function (response) {
                if (response.details != null) {
                    ...
                    for (var j = 0; j < response.details.length; j++) {
                        ...
                        jQuery("#tableLeavesUpdated").dataTable().fnAddData([j + 1, (response.details[j].Name).toString(), (response.details[j].LeaveHistory_Available).toString(), (response.details[j].LeaveHistory_SickL).toString(), (response.details[j].Updated).toString()]);
